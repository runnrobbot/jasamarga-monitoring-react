...
const PICReports = () => {
  ...
  useEffect(() => { fetchUserAP(); }, [user]);
  useEffect(() => { if (userAP) fetchKomitmen(); }, [userAP]);

  const fetchUserAP = async () => {
    try {
      if (user?.uid) {
        const snap = await getDoc(doc(db, 'users', user.uid));
        if (snap.exists()) setUserAP(snap.data().namaAP || '');
      }
    } catch (err) {
      console.error(err);
  ...
  const fetchKomitmen = async () => {
    ...
    try {
      setLoading(true);
      const snap = await getDocs(query(collection(db, 'komitmen'), where('namaAP', '==', userAP)));
  ...
  const generateExcelReport = async () => {
    try {
      ...
      const summaryData = [
        ...
        { 'Keterangan': `AP: ${userAP}`, 'Nilai': '', 'Satuan': '' },
      ...
      const filename = `Laporan_${userAP.replace(/\s/g, '_')}_${getPeriodLabel().replace(/\s/g, '_')}.xlsx`;
  ...
  const getStatusBadge = (item) => {
    ...
    if (item.approvalStatus === 'rejected') return <Badge bg="danger">Rejected - Perlu Revisi</Badge>;
              ...
              <Row>
                <Col md={6}>
                  ...
                  <ul>
                    ...
                    <li>Ringkasan per Jenis Pengadaan</li>
                  </ul>
                </Col>
                <Col md={6}>
